...
# ---------------------------------------------------------------------------
# Per-strategy means (Wilcoxon paired tests) ignore the nesting structure of
# repetitions within objects within domains. A linear mixed-effects model
# with strategy as a fixed effect and (domain, object) as random effects
# partitions variance correctly and gives a single "is strategy a real
# predictor?" coefficient + 95% CI per strategy.
#
# Model spec:
#     metric ~ strategy + (1 | domain) + (1 | domain:object)
#
# `domain:object` nests objects within their domain so the same object name
# in two different domains is not treated as the same group.
# ---------------------------------------------------------------------------
def fit_mixed_effects(
    ...
    """Fit a mixed-effects model and return a list of result rows.

    Each row: {strategy, beta, se, ci_lo, ci_hi, p_value} expressing the
    estimated mean difference vs the baseline strategy, in the metric's
    natural units (e.g. coverage ratio 0..1, validity 0/1).

    Returns an empty list if statsmodels is missing or there is too little
    ...
    for r in results:
        ...
        if metric == "coverage":
            ...
            if not cov:
                continue
            y = float(cov.get("ratio", 0.0))
        elif metric == "validity":
            v = r.get("validation", {})
            y = 1.0 if v.get("is_valid") else 0.0
        else:
        ...
        rows.append({
            ...
            "domain": r.get("domain"),
            "object": f"{r.get('domain')}::{r.get('object')}",
        })
    if not rows:
        return []
    ...
    # Set baseline as reference level for the strategy fixed effect.
    if baseline in df["strategy"].unique():
        cats = [baseline] + [s for s in df["strategy"].unique() if s != baseline]
        df["strategy"] = pd.Categorical(df["strategy"], categories=cats)

    # Random intercept per (domain::object) cell — captures both
    # domain-level and object-level baselines in a single grouping factor.
    # This is more robust than nested random effects on small samples.
    try:
        model = smf.mixedlm(
            "y ~ C(strategy)",
            data=df,
    ...
    for term, beta in fit.params.items():
        if not term.startswith("C(strategy)"):
            continue
        # term looks like: C(strategy)[T.proposed]
        m = re.match(r"C\(strategy\)\[T\.(.+?)\]$", term)
        if not m:
            continue
        s = m.group(1)
...
def print_mixed_effects(rows: list[dict], metric: str):
    if not rows:
        print(f"\n=== Mixed-effects regression ({metric}) ===")
        print("  (skipped — statsmodels not installed or insufficient data)")
        return
    base = rows[0]["vs_baseline"]
    print(f"\n=== Mixed-effects regression ({metric}, vs {base}) ===")
    print("  Model: y ~ strategy + (1 | domain::object)  -- random intercept per cell")
    print(f"  {'strategy':<26} {'beta':>8}  {'95% CI':>20}  {'p':>8}")
    for r in rows:
        ...
        if r["p_value"] < 0.001:
            ...
            sig = "*"
        print(
            f"  {r['strategy']:<26} {r['beta']:>+8.3f}  {ci:>20}  "
...
def main():
    ...
    # Mixed-effects regression on coverage and validity (raw condition).
    me_cov = fit_mixed_effects(results, "coverage", "raw")
    print_mixed_effects(me_cov, "coverage")
    me_val = fit_mixed_effects(results, "validity", "raw")
    print_mixed_effects(me_val, "validity")
    ...
    if args.csv:
        ...
        dump_csv(args.out, "mixed_effects_coverage", me_cov)
        dump_csv(args.out, "mixed_effects_validity", me_val)
