...
Each (object, strategy, repetition) produces TWO log entries:
  - condition: "raw"
  - condition: "repaired"

The repair pass uses ONLY structural feedback (BT.CPP errors). Coverage is
NEVER passed back to the LLM.
"""
...
def load_config(path: str) -> dict:
    """Load a YAML config and expand the (optional) domains_include directive.

    Sub-configs (e.g. llama.yaml) can keep only model/strategy settings and
    refer to a shared domains file via:

        domains_include: "_domains.yaml"

    The included file is resolved relative to the parent config and must
    contain a top-level `domains:` key.
    """
    with open(path) as f:
    ...
    if include and "domains" not in cfg:
        ...
        if not isinstance(sub, dict) or "domains" not in sub:
            raise ValueError(
                f"domains_include file {include_path} must define a top-level "
                f"'domains:' key, got: {type(sub).__name__}"
            )
...
def load_generic_examples(
    example_dir: str = "data/few_shot_examples",
) -> str:
    """Load SIL-unrelated generic BT examples.

    how many examples are passed to the prompt. None = load all.
    """
    ...
    for fname in sorted(os.listdir(example_dir)):
        if not fname.endswith(".xml"):
            continue
        with open(os.path.join(example_dir, fname)) as f:
...
def _validate_for_log(bt_xml: str, expected_behaviors: list) -> tuple[dict, dict]:
    """Run the full validator + coverage on a BT and return both dicts."""
    if not bt_xml:
        return (
            {"is_valid": False, "errors": ["no XML produced"], "warnings": [], "metrics": {}},
...
def _structural_validator_fn(bt_xml: str):
    """Used by structural_repair: returns None if OK, error string otherwise."""
    vr = validate_bt_xml(bt_xml)
    if vr.is_valid:
        return None
    return "\n".join(vr.errors)
...
def load_resume_state(log_path: str) -> set[tuple]:
    """Read an existing log file and return the set of completed cells.

    A cell is identified by (domain, object, strategy, rep_index). A cell
    counts as 'completed' iff BOTH the raw row and the repaired row are
    present in the log. Error rows do not count as completed, so they will
    ...
    completed = raw_seen & repaired_seen
    # Drop entries with rep_index=None — those are from older logs without
    # the rep_index field, and we cannot match them reliably.
    completed = {k for k in completed if k[3] is not None}
    return completed
...
def run_one_with_retry(
    ...
    domain_cfg: dict,
    obj: dict,
    generic_examples: str,
    repair_iterations: int,
    max_attempts: int = 3,
    backoff: float = 5.0,
) -> dict:
    """Wrap run_one with run-level retry on top of LLMClient retry.

    LLMClient already retries individual API calls on transient errors.
    This wrapper adds an outer retry for the rare case where all client
    retries are exhausted (e.g. a sustained outage during a multi-step
    proposed pipeline). On each failure we sleep ``backoff`` seconds and
    try again, up to ``max_attempts`` total.
    """
    ...
    for attempt in range(max_attempts):
        try:
            return run_one(
                ...
                generic_examples, repair_iterations,
            )
        except Exception as e:
            last_err = e
            if attempt < max_attempts - 1:
                print(f"       run_one attempt {attempt + 1}/{max_attempts} failed: {e}. "
                      f"Retrying in {backoff:.0f}s...")
                time.sleep(backoff)
            else:
                raise
    raise last_err if last_err else RuntimeError("run_one_with_retry exhausted")
...
def run_one(
    ...
    """Generate + raw-score + repair + repaired-score for one (strategy, object)."""
    ...
    if strategy == "zero_shot":
        ...
        gen = generator.few_shot_generic(obj_name, obj_desc, generic_examples,
                                         domain, sut)
    ...
    raw_validation, raw_coverage = _validate_for_log(raw_xml, expected)

    # 2. Repair pass (only if we have any XML to repair)
    if raw_xml:
        repair = generator.structural_repair(
            raw_xml,
            validator_fn=_structural_validator_fn,
    ...
    repaired_validation, repaired_coverage = _validate_for_log(repaired_xml, expected)
    ...
    return {
        ...
        "generation": gen,
        "raw": {"bt_xml": raw_xml, "validation": raw_validation, "coverage": raw_coverage},
        "repair": repair,
        "repaired": {
            "bt_xml": repaired_xml,
...
def main():
    parser = argparse.ArgumentParser(
        description="Run SIL environment object behavior repertoire generation experiments",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="LOG_PATH",
        help="Resume an interrupted run by appending to an existing JSONL log. "
             "Already-completed (domain, object, strategy, rep) cells are skipped.",
    )
    parser.add_argument(
        "--temperature",
        type=float,
        default=None,
        metavar="T",
        help="Override model.temperature from the config. Used by the "
             "temperature-sweep ablation (e.g. 0.0, 0.3, 0.7, 1.0).",
    )
    parser.add_argument(
        "--repetitions",
        type=int,
        default=None,
        metavar="N",
        help="Override experiment.repetitions from the config.",
    )
    parser.add_argument(
        "--log-suffix",
        default=None,
        metavar="STR",
        help="Append a suffix to the experiment name so concurrent sweeps "
             "(e.g. different temperatures) write to distinct log files.",
    )
    ...
    if args.log_suffix:
        exp["name"] = f"{exp['name']}_{args.log_suffix}"

    if args.dry_run:
        print(json.dumps({
            k: v for k, v in config.items() if k != "domains"
    ...
    # Resume support: if --resume is given, append to that log file and
    # skip already-completed cells. Otherwise create a fresh timestamped log.
    completed_cells: set[tuple] = set()
    if args.resume:
        if not os.path.isfile(args.resume):
            raise FileNotFoundError(f"--resume target does not exist: {args.resume}")
        completed_cells = load_resume_state(args.resume)
        print(f"Resuming from {args.resume} ({len(completed_cells)} cells already complete)")
        ...
        for strategy in strategies:
            for obj in objects:
                for rep in range(repetitions):
                    ...
                    # Resume: skip cells that are already in the log.
                    if cell in completed_cells:
                        print(
                            f"[{count}/{total}] {domain_name} | {strategy} | "
                            f"{obj['name']} | rep {rep + 1}/{repetitions}  [SKIP — resumed]"
                        )
                        continue
                    ...
                    try:
                        ...
                        print(
                            f"       raw: {'V' if rv else 'X'} cov={rc}/{rt}  "
